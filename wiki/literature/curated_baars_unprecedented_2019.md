---
type: paper
status: needs-verification
source_status: peer-reviewed
review_stage: ai-extracted
reading_status: metadata-only
topics: [remote_sensing, aerosol, wildfire]
keywords: [pyrocb, lidar]
created: 2026-05-17
updated: 2026-05-17
sources: [baars_unprecedented_2019]
---

# The Unprecedented 2017-2018 Stratospheric Smoke Event

## Bibliographic Metadata

- Citation Key: baars_unprecedented_2019
- Title: The unprecedented 2017-2018 stratospheric smoke event: decay phase and aerosol properties observed with the EARLINET
- Authors: Holger Baars et al.
- Venue/Year: Atmospheric Chemistry and Physics, 19, 15183-15198, 2019
- Status: peer-reviewed
- DOI: 10.5194/acp-19-15183-2019
- URL: https://doi.org/10.5194/acp-19-15183-2019
- Raw Source: Zotero item `VZMPA2G3`; metadata checked against Copernicus ACP record
- Reading Status: abstract-only

## Research Question

Not yet fully extracted. This paper is relevant to the decay phase and optical properties of the 2017-2018 stratospheric smoke event observed by EARLINET.

## Method

Abstract-level only: uses EARLINET lidar observations to characterize smoke-layer evolution and aerosol optical properties.

## Main Findings

- Full paper not yet summarized.

## Limitations

- Only bibliographic metadata and abstract-level framing have been captured.

## Citable Claims

- No citable claim extracted yet.

## Connections

- Parent keyword: [[keyword_stratospheric_aerosol|Stratospheric Aerosol]]
- Related concepts: pyroCb smoke, stratospheric aerosol aging, lidar depolarization
- Related methods: EARLINET lidar network observations
- Related datasets: EARLINET

## Synthesis Update Needed

- Yes: pyroCb / stratospheric smoke synthesis
- Which synthesis page: TBD

## Keywords
- Topics: remote_sensing, aerosol, wildfire
- Keywords: pyrocb, lidar

## Evidence / Figures
- Figure/table/result: Not yet extracted.
- What it supports: Not yet extracted.
- Reading limitation: See `reading_status`.

## My Notes on This Paper
- Single-paper interpretation: Not yet discussed.
- Questions for discussion: TBD.
- Possible use in my research: TBD.
