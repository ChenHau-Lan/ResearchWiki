...
新手安裝的 `python3 tools/bootstrap_rkf.py --apply --install-connector` 會從 repo
安裝 version-matched 的全域 `rkf-auto-connect` skill。Agent 使用它解析
ResearchWiki checkout，再由 `rkf.workspace.toml` 找到 live storage。它只會把
`rkf.activate` request 交給目前
task 的 session-owned runtime；preflight 不寫入任何 knowledge page。
Repo helper 以 `open_action_runtime` 建立這個 task-owned adapter：它同時綁定 connector
指向的 ResearchWiki checkout 與目前 project root，並拒絕沒有有效 marker 的外部
project。Activation、query、discovery、dashboard 與 capture 必須重用同一個 in-memory
runtime；不能把它們拆成互不相關的 helper process，否則 OFF／ACTIVE 邊界就無法成立。
...
如果要讓某專案長期記得 RKF 可用，請先用 helper preview：

```bash
python3 tools/rkf_auto_connect.py connect-project "/path/to/Project" --project-name "Project"
```

確認 receipt 後才套用：

```bash
python3 tools/rkf_auto_connect.py connect-project "/path/to/Project" --project-name "Project" --apply
```

它會建立 v2 `.rkf-connect.toml`：
...
既有 v1 marker 需要先 preview，再加 `--apply-upgrade` 才能升級；不同的 v2 policy
或未知的 future version 不會被 helper 覆寫。

Apply 會先驗證 project root、marker、`RKF/` 與四個 bridge target 都不是 symlink／
非預期類型，且需要新增內容的位置可寫。Bridge files 先完成，marker 最後才以
atomic write 建立或升級；若任一步失敗，只回滾本次能以 exact bytes 證明新建的
bridge／marker，保留所有既有 note 與 v1 marker。
...
專案層級可把 availability 與 capture mode 改成停用；這仍不影響其他專案：
