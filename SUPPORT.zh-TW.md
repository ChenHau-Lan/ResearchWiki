...
- 不知道安裝少了什麼工具。
- 可選的 `ResearchWikiCodex.command` skill/mode router 跑不起來。
- DOI dashboard、full_text index、wiki doctor 出現錯誤。
- review queue、runtime state、graph export、semantic lint 或 thesis-review output 看起來不對。
- 新手照 README 做但卡住。
...
請先讀 SUPPORT.zh-TW.md、README.zh-TW.md、USER_GUIDE.zh-TW.md 和 core/agent_contract.md，然後執行 python3 tools/support_report.py --issue-url。
...
可選的 `ResearchWikiCodex.command` router 也有 `wiki-lint/feedback-issue` advanced support mode；它只會先詢問 issue 標題、複製 Codex prompt 並開啟 Codex。完整描述、截圖與最後是否送出，會在接下來的新對話中補齊與確認。
