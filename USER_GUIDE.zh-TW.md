...
3. 選 `Open authorized source pages`，只從 publisher、作者、open-access、institutional access 或使用者已授權來源下載/確認 evidence。
4. 把合法 PDF 直接放到 `raw/doi_pdf/`。
5. 回 command 選 `Import evidence + create QCed full_text`，由本地工具從 PDF DOI metadata 建 row、改名、抽 staging text，並交給 Codex 重排/QC；成功後才寫入 `raw/full_text/` 並更新 dashboard/index。
6. 進度看 `raw/doi_dashboard.md`。
7. QC 後 full text 產生後，選 `Ingest QCed full_text to wiki`，由 Codex 產生或更新 paper page。
8. 只有 open publisher HTML/XML、授權瀏覽器 session 或真的需要來源判斷時，才用 `Codex-assisted source/full-text finding`。
...
- 第 1 項 `Add/open paper sources`：加入或打開 `raw/paper_sources.md`，可放 DOI、DOI URL、article URL、PDF URL 或來源註記。
- 第 2 項 `Open/manage DOI dashboard`：打開目前 DOI 進度看板。
- 第 3 項 `Codex-assisted source/full-text finding`：只處理例外情況，例如 open publisher HTML/XML、授權瀏覽器 session，或需要判斷合法來源路徑的 DOI/URL。遇到出版社阻擋時不要長時間硬找，應回到 source-first 流程。
- 第 4 項 `Prepare Codex app source/full-text finding prompt`：不執行 CLI；產生英文 source/full-text finding prompt，寫到 `maintenance/codex_app_handoff_prompt.md`，初始化 `maintenance/codex_app_last_run.log`，可用時複製到剪貼簿，並開啟 Codex app 到本專案。
- 第 5 項 `Open authorized source pages`：針對 source queue 或 dashboard 中沒有 PDF/full text 的 DOI 開啟 DOI/article/source page 與 `raw/doi_pdf/`。只使用 publisher、作者、open-access、institutional access 或使用者已授權 PDF/full text；本專案不自動化 shadow-library / 未授權下載。
- 第 6 項 `Import evidence + create QCed full_text`：本地維護加 Codex 轉換。會檢查 `raw/doi_pdf/` 裡新放入的 PDF，建立 missing dashboard row，改名為 `<paper_file_key>.pdf`，抽成 `raw/staging/extracted_text/<paper_file_key>.md`，再由 Codex reflow/QC；成功後才寫入 `raw/full_text/<paper_file_key>.md`、更新 dashboard/index。不建立 wiki page。
- 第 7 項 `Ingest QCed full_text to wiki`：前台執行 Codex，只從已 QC 的 `raw/full_text/` 產生或更新 `wiki/literature/` paper page；不再取得新來源，也不做 full text reflow/QC。
