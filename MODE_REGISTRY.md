...
|---|---|---:|---|
| Connect & Activate | `rkf.activate`, `rkf.status`, `rkf.deactivate`, `connect.validate` | lineage only | every task starts OFF; paths and raw prompts are excluded |
| Add | `workflow.add` | event, inbox/source projection | candidates and metadata are not stable evidence |
| Ask | `workflow.ask` | action lineage only | exact ID/title/keyword first; evidence claims require locators |
| Read | `workflow.read` | canonical Evidence | exact locator required; verification is explicit |
...
- Question over papers/topics → Ask.
...
1. Paper → locator-backed Evidence → human-reviewed Claim → Synthesis.
