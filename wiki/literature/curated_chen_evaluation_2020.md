---
type: paper
status: needs-verification
source_status: peer-reviewed
review_stage: ai-extracted
reading_status: metadata-only
topics: [remote_sensing, aerosol]
keywords: [stratospheric_aerosol, retrieval_validation]
created: 2026-05-17
updated: 2026-05-17
sources: [chen_evaluation_2020]
---

# Evaluation of the OMPS/LP Stratospheric Aerosol Extinction Product Using SAGE III/ISS Observations

## Bibliographic Metadata

- Citation Key: chen_evaluation_2020
- Title: Evaluation of the OMPS/LP stratospheric aerosol extinction product using SAGE III/ISS observations
- Authors: Zhong Chen, Pawan K. Bhartia, Omar Torres, Glen Jaross, Robert Loughman, Matthew DeLand, Peter Colarco, Robert Damadeo, Ghassan Taha
- Venue/Year: Atmospheric Measurement Techniques, 13, 3471-3485, 2020
- Status: peer-reviewed
- DOI: 10.5194/amt-13-3471-2020
- URL: https://doi.org/10.5194/amt-13-3471-2020
- Raw Source: Zotero item `6CT9QNEB`; metadata checked against Copernicus AMT record
- Reading Status: abstract-only

## Research Question

How reliable is the OMPS/LP Version 1.5 stratospheric aerosol extinction product when evaluated against SAGE III/ISS observations?

## Method

Abstract-level only: compares OMPS/LP aerosol extinction profiles with SAGE III/ISS solar occultation observations over June 2017 to May 2019.

## Main Findings

- Full paper not yet summarized.

## Limitations

- Only abstract-level method has been captured; detailed retrieval assumptions and bias structure need full-text reading.

## Citable Claims

- No citable claim extracted yet.

## Connections

- Parent keyword: [[keyword_stratospheric_aerosol|Stratospheric Aerosol]]
- Related concepts: stratospheric aerosol extinction, retrieval validation, satellite limb profiling
- Related methods: OMPS/LP retrieval, SAGE III/ISS comparison
- Related datasets: OMPS/LP, SAGE III/ISS

## Synthesis Update Needed

- Yes: remote sensing validation synthesis
- Which synthesis page: TBD

## Keywords
- Topics: remote_sensing, aerosol
- Keywords: stratospheric_aerosol, retrieval_validation

## Evidence / Figures
- Figure/table/result: Not yet extracted.
- What it supports: Not yet extracted.
- Reading limitation: See `reading_status`.

## My Notes on This Paper
- Single-paper interpretation: Not yet discussed.
- Questions for discussion: TBD.
- Possible use in my research: TBD.
